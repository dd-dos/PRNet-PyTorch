...
from utils.data import DEFAULT_CROPPED_IMAGE_SHAPE, DEFAULT_UVMAP_SHAPE, UV_COORDS, BFM
from utils.data import bfm2Mesh, mesh2UVmap, renderMesh
...
class DataProcessor:
    ...
    def runPosmap(self):
        ...
        sio.savemat(self.write_dir + '/' + self.image_name + '_bbox_info.mat',
                    {'OldBbox': old_bbox, 'Bbox': bbox, 'Tform': trans_mat, 'TformInv': trans_mat_inv,
                     'Kpt': new_kpt,
                     'OldKpt': init_kpt})
        uv_path = self.write_dir + '/' + self.image_name + '_cropped_uv_posmap.npy'
        img_path = self.write_dir + '/' + self.image_name + '_cropped.jpg'

        np.save(uv_path, uv_position_map)
        io.imsave(img_path,
                  (np.squeeze(cropped_image * 255.0)).astype(np.uint8))

        return img_path, uv_path
    ...
    def processImage(self, image_path, output_dir):
        ...
        img_path, uv_path = self.runPosmap()
        self.clear()
        return img_path, uv_path
...
def workerProcess(image_paths, output_dirs, worker_id, worker_conf):
    ...
    afw_obj = ZipFile(os.path.join("data/compressed-data", "AFW.zip"), 'w', ZIP_DEFLATED)
    helen_obj = ZipFile(os.path.join("data/compressed-data", "HELEN.zip"), 'w', ZIP_DEFLATED)
    ibug_obj = ZipFile(os.path.join("data/compressed-data", "IBUG.zip"), 'w', ZIP_DEFLATED)
    lfpw_obj = ZipFile(os.path.join("data/compressed-data", "LFPW.zip"), 'w', ZIP_DEFLATED)

    for i in tqdm.tqdm(range(len(image_paths))):
        img_path, uv_path = data_processor.processImage(image_paths[i], output_dirs[i])
        ...
        elif 'LFPW' in img_path:
            lfpw_obj.write(img_path)
            lfpw_obj.write(uv_path)
